...
Description: LoRa MAC layer implementation Lora MAC 层实现
...
/*!
 * Maximum MAC commands buffer size MAC命令最大长度
 */
#define LORA_MAC_COMMAND_MAX_LENGTH                 128

/*!
 * Maximum length of the fOpts field fOpts 帧选项(fOpts）最大字节
 */
...
/*!
 * LoRaMac duty cycle for the back-off procedure during the first hour 一个小时内回退程序占空比 .
 */
#define BACKOFF_DC_1_HOUR                           100

/*!
 * LoRaMac duty cycle for the back-off procedure during the next 10 hours 10个小时内回退程序占空比.
 */
#define BACKOFF_DC_10_HOURS                         1000

/*!
 * LoRaMac duty cycle for the back-off procedure during the next 24 hours 24个小时内回退程序占空比.
 */
#define BACKOFF_DC_24_HOURS                         10000

#ifdef CONFIG_LORA_CAD  //是否使能CAD(信道空闲检测)
#define LORA_CAD_CNT_MAX 8    //send frame after LORA_CAD_CNT_MAX times CAD CAD最大检测次数
#define LORA_CAD_SYMBOLS 8   //CAD symbols  CAD 符号？ 
#define LORA_CAD_DELAY  1   //delay 1ms after CAD 延时一秒后开始CAD检测
#endif

/*!
 * Device IEEE EUI  设备EUI
 */
static uint8_t *LoRaMacDevEui;

/*!
 * Application IEEE EUI APP EUI
 */
static uint8_t *LoRaMacAppEui;

/*!
 * AES encryption/decryption cipher application key 应用KEY
 */
static uint8_t *LoRaMacAppKey;

/*! 
 * AES encryption/decryption cipher network session key 网络会话密钥
 */
...
/*!
 * AES encryption/decryption cipher application session key 应用会话密钥
 */
...
/*!
 * Device nonce is a random value extracted by issuing a sequence of RSSI 随机数
 * measurements
 */
...
/*!
 * Multicast channels linked list 多播信道列表
 */
static MulticastParams_t *MulticastChannels = NULL;

/*!
 * Actual device class 设备类型  A、B、C?
 */
static DeviceClass_t LoRaMacDeviceClass;

/*!
 * Indicates if the node is connected to a private or public network 是私有还是公有网络
 */
static bool PublicNetwork;

/*!
 * Buffer containing the data to be sent or received.发送、接收数据的共用缓存数组
 */
static uint8_t LoRaMacBuffer[LORAMAC_PHY_MAXPAYLOAD];

/*!
 * Length of packet in LoRaMacBuffer 缓存数据长度
 */
static uint16_t LoRaMacBufferPktLen = 0;

/*!
 * Length of the payload in LoRaMacBuffer发送负载长度
 */
static uint8_t LoRaMacTxPayloadLen = 0;

/*!
 * Buffer containing the upper layer data.接收数据中的负载数据
 */
...
/*!
 * Used for test purposes. Disables the opening of the reception windows.用于测试，禁止打开接收窗口或者使能接收窗口
 */
static bool IsRxWindowsEnabled = true;

/*!
 * Indicates if the MAC layer has already joined a network.用于标志是否已经加入网络
 */
...
/*!
 * Counts the number of missed ADR acknowledgements  计算ADR应答的计数器
 */
