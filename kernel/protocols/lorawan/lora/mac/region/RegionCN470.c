...
bool RegionCN470TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    *txPower = txPowerLimited;
		frequency = Channels[txConfig->Channel].Frequency;
		DBG_LINKWAN("Tx, Power: %d, Freq: %u,DR: %d, len: %u, at %llu\r\n",phyTxPower,(unsigned int)frequency, txConfig->Datarate, txConfig->PktLen, *txTimeOnAir);
    return true;
}
